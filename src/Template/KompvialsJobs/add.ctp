...
$( document ).ready(function() {
    $.getScript('/js/find-komp-vial.js', function(){
        //Inject hidden fields and submit a form    
        $('#submit-komp-vial-button').click(function(event) {
            event.preventDefault();
            var form = $( '#find-vial-form');
            el  = "<input class='hidden' type='text' name='job_id' value='<?= $jobId ?>'>";
            el += "<input class='hidden' type='text' name='komp_vial_id' value='" + selectedKompVialId +"'>";
            el += "<input class='hidden' type='text' name='komp_order_id' value='" + selectedKompOrderId +"'>";
            form.append(el);
            form.submit();
        });
    });
...
<?= $this->Form->create($kompvialsJob, ['id'=>'find-vial-form']) ?>
